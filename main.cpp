...
void urediOdgovore(vector <klasa_stablo> vektor_pitanja){
    ...
    for (int i = 0; i < vektor_pitanja.size(); i++) {
        ...
        while(!node_queue.empty()) {
            next = node_queue.front();
            node_queue.pop();
            for (int i = 0; i<next.sinovi.size()-1; i++){
                for (int j = 0; j < next.sinovi.size()-i-1; j++){
                    if(next.sinovi[j]->ocena>next.sinovi[j+1]->ocena){
                        swap(next.sinovi[j], next.sinovi[j+1]); //proveri ako moze swap, ako ne onda samo cepaj sa tmp
                    }
                }
            }
            if (!next.sinovi.empty()) {

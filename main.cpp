...
/*napravi novu funkciju gde trazi pokazivac na oca, ako je next od nekog cvora taj i taj
 * onda stavis tom i tom da je pokazivac na oca, probaj to sutra*/
...
void obrisiOdgovoriliKomentar(string question, string answer, vector <klasa_stablo>& vektor_pitanja){
    klasa_stablo koren, next, otac_cvora;
    queue<klasa_stablo> node_queue;
    ...
    while(!node_queue.empty()) {
        ...
        node_queue.pop();
        if (next.recenica == answer) {
            otac_cvora=*next.pokazivac_na_oca[0];
        }
        if (!next.sinovi.empty()) {
            for (int i = 0; i < next.sinovi.size(); i++) {
                node_queue.push(*next.sinovi[i]);
    ...
    for(int i=0; i<otac_cvora.sinovi.size(); i++){
        if(otac_cvora.sinovi[i]->recenica==answer){
            otac_cvora.sinovi.erase(otac_cvora.sinovi.begin()+i);
        }
    }
}
